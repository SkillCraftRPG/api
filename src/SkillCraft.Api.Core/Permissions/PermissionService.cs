...
internal class PermissionService : IPermissionService
{
  ...
  public async Task CheckAsync(string action, IEntityProvider? resource, CancellationToken cancellationToken)
  {
    ...
    else
    {
      isAllowed = (entity?.Kind) switch
      {
        ...
        Caste.EntityKind or Customization.EntityKind or Education.EntityKind or Party.EntityKind => IsAllowed(action, entity),
  ...
  {
    Actions.CreateCaste or Actions.CreateCustomization or Actions.CreateEducation or Actions.CreateParty => _context.IsWorldOwner,
